...
export default function Sidebar({ role = 'user' }: { role?: 'admin' | 'user' | 'agent' }) {
  ...
  const [vulnCounts, setVulnCounts] = useState<{ critical: number; high: number; medium: number; low: number } | null>(null)
    ...
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: '4px 10px', display: 'flex', flexDirection: 'column', gap: 1, overflowY: 'auto' }}>
        {nav.map(({ label, href, icon: Icon }) => {
          ...
          const badge =
            href === '/scans'   && scanTotal   != null ? String(scanTotal) :
            href === '/reports' && reportTotal != null && reportTotal > 0 ? String(reportTotal) :
            null
          const isAlert = false
          return (
      ...
      {(() => {
        ...
        const rawRisk = Math.min(100, Math.round((c * 14 + h * 7 + m * 2.5 + l * 0.8) / 1.4))
        const level =
          rawRisk === 0   ? { label: 'NONE',     color: '#94a3b8', barColor: '#94a3b8',                                   pct: 4  } :
          rawRisk <= 20   ? { label: 'LOW',      color: '#00cc88', barColor: '#00cc88',                                   pct: 20 } :
          rawRisk <= 50   ? { label: 'MEDIUM',   color: '#ffcc00', barColor: 'linear-gradient(90deg,#ffcc00,#ff6b35)',     pct: 50 } :
          rawRisk <= 75   ? { label: 'HIGH',     color: '#ff6b35', barColor: 'linear-gradient(90deg,#ffcc00,#ff6b35)',     pct: 75 } :
                            { label: 'CRITICAL', color: '#ff3355', barColor: 'linear-gradient(90deg,#ffcc00,#ff6b35,#ff3355)', pct: 100 }
        return (
          <div style={{ padding: '0 10px', marginBottom: 10 }}>
            <div style={{ padding: '10px 12px', borderRadius: 9, background: `${level.color}08`, border: `1px solid ${level.color}20` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: level.color, fontWeight: 700 }}>{level.label}</span>
              </div>
              <div style={{ height: 3, background: 'var(--border-default)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ width: `${rawRisk || 4}%`, height: '100%', background: level.barColor, borderRadius: 3, boxShadow: `0 0 8px ${level.color}66`, transition: 'width 1s cubic-bezier(0.16,1,0.3,1)' }} />
              </div>
            </div>
          </div>
        )
