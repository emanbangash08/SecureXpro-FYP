...
import { RefreshCw, X, AlertTriangle, CheckCircle2, AlertCircle, Shield, Clock, Target, User, Search, Filter, Zap } from 'lucide-react'
import { api, type AdminScan } from '@/lib/api'

const STATUS_COLOR: Record<string, { color: string; bg: string }> = {
  completed:  { color: '#00cc88', bg: 'rgba(0,204,136,0.1)'  },
  running:    { color: '#00e5cc', bg: 'rgba(0,229,204,0.1)'  },
  pending:    { color: '#ffcc00', bg: 'rgba(255,204,0,0.1)'  },
  failed:     { color: '#ff3355', bg: 'rgba(255,51,85,0.1)'  },
  cancelled:  { color: '#4a5568', bg: 'rgba(74,85,104,0.1)'  },
}
...
function ScanDetailModal({ scanId, onClose }: { scanId: string; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, padding: '24px 24px', overflowY: 'auto' }} onClick={onClose}>
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  { icon: User,    label: 'Owner',        value: `${scan.username} <${scan.user_email}>` },
                  { icon: Shield,  label: 'Type',         value: scan.scan_type.replace('_', ' ') },
...
const EMPTY_FILTERS: Filters = {
  search: '', status: '', scan_type: '', risk: '', date_from: '', date_to: '', has_exploits: '',
}
...
export default function AdminScansPage() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto', fontFamily: 'var(--font-ui)' }}>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-strong)', fontFamily: 'var(--font-display)', letterSpacing: '-.5px', marginBottom: 4 }}>Scan Management</h1>
          <p style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>All scans across all users — {total} total</p>
        </div>
      ...
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 14, padding: '16px 20px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
          ...
          <select value={filters.scan_type} onChange={e => set('scan_type', e.target.value)} style={selectStyle}>
            <option value="">All Types</option>
            {SCAN_TYPES.filter(Boolean).map(t => <option key={t} value={t}>{t.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</option>)}
          </select>

          <select value={filters.risk} onChange={e => set('risk', e.target.value)} style={selectStyle}>
            <option value="">Any Risk</option>
            {RISK_LEVELS.filter(Boolean).map(r => <option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
          </select>

          <select value={filters.has_exploits} onChange={e => set('has_exploits', e.target.value as Filters['has_exploits'])} style={selectStyle}>
        ...
        {/* Row 2: date range + active filter count + clear */}
      ...
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 14, overflow: 'hidden' }}>
        {/* Header row */}
        <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 110px 120px 1fr 90px 90px 100px 110px 90px', background: 'var(--bg-overlay)', padding: '12px 20px', borderBottom: '1px solid var(--border-subtle)', gap: 12 }}>
          {['Target', 'Type', 'Status', 'Owner', 'Vulns', 'Risk', 'Started', 'Completed', 'Details'].map(h => (
        ...
        {loading ? (
          ...
          return (
            <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1.6fr 110px 120px 1fr 90px 90px 100px 110px 90px', alignItems: 'center', padding: '13px 20px', borderBottom: i < scans.length - 1 ? '1px solid var(--border-subtle)' : 'none', gap: 12, transition: 'background .15s', cursor: 'default' }}
              onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = 'var(--bg-elevated)'}
              onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}>

              {/* Target */}
              <div>
                <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.target}</div>
                <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', marginTop: 1 }}>{s.id.slice(-8)}</div>
              </div>

              {/* Type */}
              <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', padding: '3px 7px', borderRadius: 5, background: `${tc.color}10`, color: tc.color, border: `1px solid ${tc.color}25`, textTransform: 'uppercase', fontWeight: 700, whiteSpace: 'nowrap' }}>
                {s.scan_type.replace('_', ' ')}
              </span>

              {/* Status */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: st.color, flexShrink: 0, boxShadow: s.status === 'running' ? `0 0 6px ${st.color}` : 'none' }} />
                <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: st.color, fontWeight: 600, textTransform: 'uppercase' }}>{s.status}</span>
              </div>

              {/* Owner */}
              <div>
                <div style={{ fontSize: 12, fontFamily: 'var(--font-display)', fontWeight: 600, color: 'var(--text-soft)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.username}</div>
                <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.user_email}</div>
              </div>

              {/* Vuln count */}
              <div style={{ fontSize: 13, fontFamily: 'var(--font-display)', fontWeight: 700, color: s.vuln_count > 0 ? '#ff6b35' : 'var(--text-muted)', textAlign: 'center' }}>
                {s.vuln_count}
              </div>
              ...
              <div style={{ textAlign: 'center' }}>
                {risk ? (
                  <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', padding: '3px 7px', borderRadius: 5, background: `${SEV_COLOR[risk] ?? '#4a5568'}10`, color: SEV_COLOR[risk] ?? 'var(--text-muted)', border: `1px solid ${SEV_COLOR[risk] ?? '#4a5568'}25`, textTransform: 'uppercase', fontWeight: 700 }}>
                    {risk}
                  </span>
                ) : (
              ...
              <button onClick={() => setDetailId(s.id)}
                style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(0,229,204,0.06)', border: '1px solid rgba(0,229,204,0.2)', color: 'var(--accent-text)', fontSize: 10, fontFamily: 'var(--font-mono)', cursor: 'pointer', fontWeight: 600, transition: 'all .2s', whiteSpace: 'nowrap' }}>
                View
              </button>
            </div>
          )
