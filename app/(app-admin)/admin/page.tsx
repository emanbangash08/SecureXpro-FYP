...
import {
  ...
  RefreshCw, ArrowRight, UserCheck, UserX,
} from 'lucide-react'
      ...
      {error ? (
        ...
        <>
          ...
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 90px 90px 110px', background: 'var(--surface-input)', padding: '10px 24px', borderBottom: '1px solid var(--border-subtle)', gap: 16 }}>
            ...
            {users.slice(0, 8).map((u, i) => (
              <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 90px 90px 110px', alignItems: 'center', padding: '13px 24px', borderBottom: i < Math.min(users.length, 8) - 1 ? '1px solid var(--border-subtle)' : 'none', gap: 16 }}>
                <div>
                ...
                <div>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, fontFamily: 'var(--font-mono)', color: STATUS_COLOR[u.status] ?? 'var(--text-muted)' }}>
