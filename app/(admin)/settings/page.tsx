...
export default function SettingsPage() {
  ...
  const labelStyle: React.CSSProperties = { display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: '#8899aa', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }
        ...
        <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 14, padding: 32 }}>
          ...
          {activeTab === 'general' && (
            <div>
              ...
              {loadingSettings ? (
                <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: '#4a5568' }}>Loading…</div>
              ) : (
          ...
          {activeTab === 'users' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                ...
                <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: '#4a5568' }}>{users.length} users</span>
              </div>
              {loadingUsers ? (
                <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: '#4a5568' }}>Loading users…</div>
              ) : (
                <div style={{ border: '1px solid rgba(255,255,255,.06)', borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px 100px 120px', background: '#050709', padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,.05)', gap: 16 }}>
                    {['Username', 'Email', 'Role', 'Status', 'Actions'].map(h => (
                      <div key={h} style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: '#4a5568', textTransform: 'uppercase' }}>{h}</div>
                    ))}
                  </div>
                  {users.length === 0 ? (
                    <div style={{ padding: '32px', textAlign: 'center', color: '#4a5568', fontSize: 13, fontFamily: 'var(--font-mono)' }}>No users found.</div>
                  ) : users.map(u => (
                    <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px 100px 120px', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,.04)', gap: 16 }}>
                      <div style={{ fontSize: 13, fontFamily: 'var(--font-display)', fontWeight: 600, color: '#e8edf5' }}>{u.username}</div>
                      <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: '#8899aa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
                      <div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => handleUpdateUserStatus(u.id, u.status === 'active' ? 'inactive' : 'active')}
                          style={{ background: 'none', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6, color: '#8899aa', cursor: 'pointer', padding: '5px 10px', fontSize: 10, fontFamily: 'var(--font-mono)', transition: 'all .2s' }}
                        ...
                        <button onClick={() => handleDeleteUser(u.id, u.username)}
                          style={{ background: 'none', border: 'none', color: '#4a5568', cursor: 'pointer', padding: '5px', transition: 'all .2s' }}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 600 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#050709', padding: '16px 20px', borderRadius: 10, border: '1px solid rgba(255,255,255,.06)' }}>
                  ...
                  <div onClick={() => setSettings(s => ({ ...s, email_notifications: !s.email_notifications }))} style={{ width: 44, height: 24, borderRadius: 12, background: settings.email_notifications ? '#00e5cc' : '#4a5568', position: 'relative', cursor: 'pointer', transition: 'background .2s', flexShrink: 0 }}>
                ...
                {([
                  ...
                  <label key={opt.key} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer' }}
                    ...
                    <div style={{ width: 18, height: 18, borderRadius: 4, border: `1px solid ${settings[opt.key] ? '#00e5cc' : '#4a5568'}`, background: settings[opt.key] ? 'rgba(0,229,204,.1)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2, transition: 'all .2s', flexShrink: 0 }}>
