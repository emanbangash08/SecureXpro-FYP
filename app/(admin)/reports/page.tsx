...
const FORMAT_COLORS: Record<string, string> = {
  pdf: '#ff6b35', html: '#4d9eff', json: '#00cc88',
}

function GenerateReportModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#06090f', border: '1px solid rgba(0,229,204,0.2)', borderRadius: 18, width: '100%', maxWidth: 520, overflow: 'hidden', boxShadow: '0 24px 80px rgba(0,0,0,0.7)', animation: 'fade-in-up 0.2s ease' }}>
        ...
        <div style={{ padding: '24px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontSize: 16, fontFamily: 'var(--font-display)', fontWeight: 700, color: '#fff' }}>Generate Report</h2>
        ...
        <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-dim)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Select Scan</label>
            {loadingScans ? (
              <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--text-faintest)' }}>Loading scans…</div>
            ) : (
              <select value={selectedScan} onChange={e => setSelectedScan(e.target.value)}
                style={{ width: '100%', background: 'var(--surface-input)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: '11px 14px', color: 'var(--text-body)', fontSize: 13, fontFamily: 'var(--font-display)', outline: 'none', cursor: 'pointer' }}>
                <option value="">— Choose a completed scan —</option>
                {scans.filter(s => s.status === 'completed').map(s => (
                  <option key={s.id} value={s.id}>{s.target} ({s.scan_type})</option>
                ))}
              </select>
            )}
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-dim)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Report Title</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Q2 Network Security Assessment"
              style={{ width: '100%', background: 'var(--surface-input)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: '11px 14px', color: 'var(--text-body)', fontSize: 13, fontFamily: 'var(--font-display)', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-dim)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Format</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {(['json', 'html', 'pdf'] as const).map(f => (
                <button key={f} onClick={() => setFormat(f)} style={{ flex: 1, padding: '10px', borderRadius: 8, border: `1px solid ${format === f ? FORMAT_COLORS[f] + '50' : 'var(--border-default)'}`, background: format === f ? `${FORMAT_COLORS[f]}10` : 'transparent', color: format === f ? FORMAT_COLORS[f] : 'var(--text-fainter)', fontSize: 12, fontFamily: 'var(--font-mono)', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', transition: 'all .2s' }}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            ...
            <button onClick={submit} disabled={!selectedScan || !title.trim() || loading}
              style={{ flex: 2, padding: '12px', borderRadius: 9, background: (!selectedScan || !title.trim()) ? 'rgba(0,229,204,0.2)' : '#00e5cc', border: 'none', color: '#020a08', fontSize: 13, fontFamily: 'var(--font-display)', fontWeight: 700, cursor: (!selectedScan || !title.trim()) ? 'not-allowed' : 'pointer', transition: 'all .2s' }}>
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400, fontFamily: 'var(--font-ui)', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Total Reports', val: reports.length,                                          icon: FileArchive, col: '#4d9eff' },
          { label: 'JSON Reports',  val: reports.filter(r => r.format === 'json').length,         icon: FileText,    col: '#00cc88' },
          { label: 'HTML Reports',  val: reports.filter(r => r.format === 'html').length,         icon: Globe,       col: '#4d9eff' },
          { label: 'PDF Reports',   val: reports.filter(r => r.format === 'pdf').length,          icon: FileText,    col: '#ff6b35' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', background: 'var(--surface-input)', borderRadius: 10, padding: 3, border: '1px solid rgba(255,255,255,0.06)' }}>
          {(['all', 'json', 'html', 'pdf'] as const).map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: typeFilter === t ? 'rgba(0,229,204,0.1)' : 'transparent', color: typeFilter === t ? '#00e5cc' : 'var(--text-faintest)', fontFamily: 'var(--font-mono)', fontSize: 11, cursor: 'pointer', transition: 'all .2s', textTransform: 'uppercase', boxShadow: typeFilter === t ? 'inset 0 0 0 1px rgba(0,229,204,0.25)' : 'none' }}>
              {t}
            </button>
          ))}
        </div>
      ...
      {/* Report Cards */}
      {loading ? (
        <div style={{ padding: '48px', textAlign: 'center', color: 'var(--text-faintest)', fontSize: 13, fontFamily: 'var(--font-mono)' }}>Loading reports…</div>
      ) : filtered.length === 0 ? (
        <div style={{ padding: '48px', textAlign: 'center', color: 'var(--text-faintest)', fontSize: 13, fontFamily: 'var(--font-mono)' }}>
          {reports.length === 0 ? 'No reports yet. Generate one from a completed scan.' : 'No reports match your filters.'}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((rep, i) => (
            <div key={rep.id}
              ...
              <div style={{ width: 36, height: 36, borderRadius: 9, background: 'rgba(77,158,255,0.1)', border: '1px solid rgba(77,158,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-faintest)' }}>
                    <Clock size={9} /> {new Date(rep.created_at).toLocaleDateString()}
                  </div>
                  <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-faintest)' }}>
                    Scan: <span style={{ color: 'var(--accent-text)' }}>{rep.scan_id.slice(0, 8)}…</span>
                  </div>
                </div>
              </div>

              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 7, background: `${FORMAT_COLORS[rep.format] || 'var(--text-dim)'}10`, border: `1px solid ${FORMAT_COLORS[rep.format] || 'var(--text-dim)'}20` }}>
                <FileText size={11} color={FORMAT_COLORS[rep.format] || 'var(--text-dim)'} />
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: FORMAT_COLORS[rep.format] || 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase' }}>{rep.format}</span>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: rep.generated ? '#00cc88' : 'var(--text-faintest)' }} />
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: rep.generated ? '#00cc88' : 'var(--text-faintest)' }}>{rep.generated ? 'Ready' : 'Pending'}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  style={{ padding: '7px', borderRadius: 7, background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', color: deleting === rep.id ? '#ff3355' : 'var(--text-faintest)', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all .2s' }}
                  ...
                  onMouseLeave={e => { if (deleting !== rep.id) { e.currentTarget.style.color = 'var(--text-faintest)'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'var(--border-default)' } }}>
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
      ...
      {showModal && <GenerateReportModal onClose={() => setShowModal(false)} onCreated={load} />}
