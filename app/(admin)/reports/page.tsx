...
const SEV_COLOR: Record<string, string> = {
  critical: '#ff3355', high: '#ff6b35', medium: '#ffcc00', low: '#00cc88', unknown: '#4a5568',
}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#06090f', border: '1px solid rgba(0,229,204,0.2)', borderRadius: 18, width: '100%', maxWidth: 520, overflow: 'hidden', boxShadow: '0 24px 80px rgba(0,0,0,0.7)', animation: 'fade-in-up 0.2s ease' }}>
        ...
        <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: '#8899aa', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Select Scan</label>
            {loadingScans ? (
              <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: '#4a5568' }}>Loading scans…</div>
            ) : (
              <select value={selectedScan} onChange={e => setSelectedScan(e.target.value)}
                style={{ width: '100%', background: '#050709', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: '11px 14px', color: '#e8edf5', fontSize: 13, fontFamily: 'var(--font-display)', outline: 'none', cursor: 'pointer' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: '#8899aa', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Report Title</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Q2 Network Security Assessment"
              style={{ width: '100%', background: '#050709', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: '11px 14px', color: '#e8edf5', fontSize: 13, fontFamily: 'var(--font-display)', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: '#8899aa', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>Format</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {(['json', 'html', 'pdf'] as const).map(f => (
                <button key={f} onClick={() => setFormat(f)} style={{ flex: 1, padding: '10px', borderRadius: 8, border: `1px solid ${format === f ? FORMAT_COLORS[f] + '50' : 'rgba(255,255,255,.08)'}`, background: format === f ? `${FORMAT_COLORS[f]}10` : 'transparent', color: format === f ? FORMAT_COLORS[f] : '#6a7b8a', fontSize: 12, fontFamily: 'var(--font-mono)', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', transition: 'all .2s' }}>
                  {f}
                </button>
              ))}
            </div>
...
export default function ReportsPage() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400, fontFamily: 'var(--font-ui)', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', background: 'rgba(0,0,0,0.4)', borderRadius: 10, padding: 3, border: '1px solid rgba(255,255,255,0.06)' }}>
          {(['all', 'json', 'html', 'pdf'] as const).map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: typeFilter === t ? 'rgba(0,229,204,0.1)' : 'transparent', color: typeFilter === t ? '#00e5cc' : '#4a5568', fontFamily: 'var(--font-mono)', fontSize: 11, cursor: 'pointer', transition: 'all .2s', textTransform: 'uppercase', boxShadow: typeFilter === t ? 'inset 0 0 0 1px rgba(0,229,204,0.25)' : 'none' }}>
              {t}
            </button>
          ))}
        </div>
      ...
      {/* Report Cards */}
      {loading ? (
        <div style={{ padding: '48px', textAlign: 'center', color: '#4a5568', fontSize: 13, fontFamily: 'var(--font-mono)' }}>Loading reports…</div>
      ) : filtered.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((rep, i) => (
            <div key={rep.id}
              style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 13, padding: '18px 20px', display: 'grid', gridTemplateColumns: '36px 1fr 110px 90px 110px', alignItems: 'center', gap: 16, transition: 'all .2s', animation: `fade-in-up ${0.05 + i * 0.04}s ease forwards` }}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: '#4a5568' }}>
                    Scan: <span style={{ color: '#00e5cc' }}>{rep.scan_id.slice(0, 8)}…</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 7, background: `${FORMAT_COLORS[rep.format] || '#8899aa'}10`, border: `1px solid ${FORMAT_COLORS[rep.format] || '#8899aa'}20` }}>
                ...
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: FORMAT_COLORS[rep.format] || '#8899aa', fontWeight: 700, textTransform: 'uppercase' }}>{rep.format}</span>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: rep.generated ? '#00cc88' : '#4a5568' }} />
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: rep.generated ? '#00cc88' : '#4a5568' }}>{rep.generated ? 'Ready' : 'Pending'}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  style={{ padding: '7px', borderRadius: 7, background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', color: deleting === rep.id ? '#ff3355' : '#4a5568', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all .2s' }}
                  ...
                  onMouseLeave={e => { if (deleting !== rep.id) { e.currentTarget.style.color = '#4a5568'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)' } }}>
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
