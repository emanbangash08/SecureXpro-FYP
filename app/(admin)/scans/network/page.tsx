...
function NetworkMapExpanded({
  ...
  vulns.forEach((v: any) => { if (v.severity in sev) (sev as any)[v.severity]++; });
  const maxCvss = vulns.reduce((m: number, v: any) => Math.max(m, v.cvss_score ?? 0), 0);

  const hostVulns = selectedHost
    ? vulns.filter((v: any) => v.affected_host === selectedHost.ip || v.affectedHost === selectedHost.ip)
    : [];
  ...
  return (
    <div style={{ display: "flex", flex: 1, minHeight: 0, overflow: "hidden" }}>
      ...
      {/* ── Large SVG map ── */}
      <div style={{ flex: 1, minWidth: 0, padding: "16px 20px 8px", display: "flex", flexDirection: "column" }}>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", flex: 1 }}>
          <defs>
            ...
            <filter id="expGlowStrong" x="-60%" y="-60%" width="220%" height="220%">
          ...
          {Array.from({ length: 6 }, (_, row) =>
            Array.from({ length: 10 }, (_, col) => (
              <circle key={`gd-${row}-${col}`} cx={col * 66 + 16} cy={row * 54 + 18}
                r="1" fill="rgba(0,229,204,0.06)" />
            ))
          )}
          ...
          {count === 0 && (
            <text x={W / 2} y={H / 2} textAnchor="middle"
              fill="rgba(100,116,130,0.5)" fontSize="12" fontFamily="monospace">
          ...
          {hosts.map((h, i) => {
            ...
            return (
              <g key={`el-${h.ip}`}>
                <line x1={scanX} y1={scanY - 22} x2={x} y2={hostY + nodeR + 2}
                  stroke={col} strokeWidth={isSel ? "1.4" : "0.8"} opacity={isSel ? 0.35 : 0.15} />
                <line x1={scanX} y1={scanY - 22} x2={x} y2={hostY + nodeR + 2}
                  stroke={col} strokeWidth={isSel ? "1.8" : "1.2"} strokeDasharray="5 10" opacity={isSel ? 0.9 : 0.55}
                  style={{ animation: `dash-flow ${1.7 + i * 0.3}s linear infinite` }} />
                <circle r="2.8" fill={col} filter="url(#expGlow)">
                  <animateMotion dur={`${2.2 + i * 0.35}s`} repeatCount="indefinite"
          ...
          {hosts.map((h, i) => {
            ...
            const shortHost = rawHost.length > 14 ? rawHost.slice(0, 13) + "…" : rawHost;

            return (
              <g key={`eh-${h.ip}`} onClick={() => onSelectHost(isSel ? null : h)}
                style={{ cursor: "pointer" }}>
                {/* Selected highlight glow */}
                {isSel && (
                  <circle cx={x} cy={hostY} r={nodeR + 12} fill="none"
                    stroke={col} strokeWidth="2" opacity="0.5"
                    style={{ animation: "topo-ring 1.8s ease-out infinite", transformOrigin: `${x}px ${hostY}px` }} />
                )}
                {/* Alert ring */}
                {isAlert && !isSel && (
                  <circle cx={x} cy={hostY} r={nodeR + 9} fill="none"
                    stroke={col} strokeWidth="1.2" opacity="0.3"
                    style={{ animation: "topo-ring 2.2s ease-out infinite", transformOrigin: `${x}px ${hostY}px` }} />
                )}
                {/* Main circle */}
                <circle cx={x} cy={hostY} r={nodeR}
                  fill={isSel ? `${col}30` : `${col}18`}
                  stroke={col} strokeWidth={isSel ? 2.2 : 1.6}
                  filter={isAlert || isSel ? "url(#expGlowStrong)" : "url(#expGlow)"} />
                {/* OS abbreviation */}
                <text x={x} y={hostY - (nodeR > 25 ? 5 : 3)} textAnchor="middle"
                  fill={col} fontSize={nodeR > 25 ? 10 : 8} fontFamily="monospace" fontWeight="800">
                  {abbr}
                </text>
                {/* Stats: vulns · ports */}
                <text x={x} y={hostY + (nodeR > 25 ? 8 : 6)} textAnchor="middle"
                  fill={col} fontSize={nodeR > 25 ? 8 : 6.5} fontFamily="monospace" opacity="0.82">
                  {vulnCnt}v · {portCnt}p
                </text>
                {/* IP */}
                <text x={x} y={hostY + nodeR + 15} textAnchor="middle"
                  fill={isSel ? "rgba(220,230,240,0.95)" : "rgba(148,163,184,0.85)"}
                  fontSize="8.5" fontFamily="monospace" fontWeight={isSel ? "700" : "400"}>
                  {ip}
                </text>
                {/* Hostname */}
                {shortHost && (
                  <text x={x} y={hostY + nodeR + 27} textAnchor="middle"
                    fill={isSel ? "rgba(180,195,210,0.8)" : "rgba(100,116,130,0.65)"}
          ...
          {/* Scanner node */}
          <g>
            <circle cx={scanX} cy={scanY} r="26" fill="none"
              stroke="rgba(0,229,204,0.18)" strokeWidth="1.8"
              style={{ animation: "topo-pulse 2.6s ease-in-out infinite" }} />
            <circle cx={scanX} cy={scanY} r="19" fill="rgba(0,229,204,0.12)"
              stroke="#00e5cc" strokeWidth="1.8" filter="url(#expGlow)" />
            <text x={scanX} y={scanY - 2} textAnchor="middle" fill="#00e5cc"
              fontSize="10" fontFamily="monospace" fontWeight="800">SXP</text>
            <text x={scanX} y={scanY + 11} textAnchor="middle" fill="#00e5cc"
              fontSize="7" fontFamily="monospace" opacity="0.7">SCANNER</text>
          </g>
        </svg>
          ...
          {[
            ...
            <div key={label} style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: col,
                boxShadow: `0 0 5px ${col}88`, flexShrink: 0, display: "inline-block" }} />
              <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)",
                textTransform: "uppercase", letterSpacing: "0.6px" }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        background: "rgba(0,0,0,0.18)", overflowY: "auto", padding: 20,
        display: "flex", flexDirection: "column", gap: 14,
      }}>
        {!selectedHost ? (
          /* Network overview */
          <>
            <div style={{ fontSize: 11, fontFamily: "var(--font-display)", fontWeight: 700, color: "var(--text-body)", marginBottom: 2 }}>
              Network Overview
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                ...
                { label: "Max CVSS",    val: maxCvss > 0 ? maxCvss.toFixed(1) : "—", col: "#ff6b35" },
              ].map(({ label, val, col }) => (
                <div key={label} style={{ background: "var(--surface-2)", borderRadius: 10, padding: "12px 14px", border: `1px solid ${col}18` }}>
                  <div style={{ fontSize: 18, fontFamily: "var(--font-display)", fontWeight: 800, color: col, lineHeight: 1 }}>{val}</div>
                  <div style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", textTransform: "uppercase", letterSpacing: "0.6px", marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", marginTop: 2 }}>Severity breakdown</div>
            {[
              ...
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: col, width: 50 }}>{label}</span>
                <div style={{ flex: 1, height: 5, borderRadius: 3, background: "var(--surface-3)", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${vulns.length > 0 ? Math.max(3, (sc / vulns.length) * 100) : 0}%`,
                    background: col, borderRadius: 3 }} />
                </div>
                <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: col, width: 20, textAlign: "right" }}>{sc}</span>
              </div>
            ))}
            <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 10, background: "rgba(0,229,204,0.05)",
              border: "1px solid rgba(0,229,204,0.12)", fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", lineHeight: 1.6 }}>
              Click any host node on the map to view detailed host information, open ports, and associated vulnerabilities.
            </div>
          </>
        ) : (
          /* Host detail view */
          <>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 15, fontFamily: "var(--font-mono)", fontWeight: 800, color: SEV_COLOR[selectedHost.severity ?? "info"] }}>
                  {selectedHost.ip}
                </span>
              ...
              {selectedHost.os && (
                <div style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)" }}>{selectedHost.os}</div>
              )}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
              {[
                { label: "Ports",  val: selectedHost.ports?.length ?? 0,  col: "#4d9eff" },
                { label: "Vulns",  val: selectedHost.vulnCount ?? 0,       col: SEV_COLOR[selectedHost.severity ?? "info"] },
                { label: "CVSS",   val: hostVulns.length > 0 ? Math.max(...hostVulns.map((v: any) => v.cvss_score ?? 0)).toFixed(1) : "—", col: "#ffcc00" },
              ].map(({ label, val, col }) => (
                <div key={label} style={{ background: "var(--surface-2)", borderRadius: 9, padding: "10px 10px 8px", textAlign: "center", border: `1px solid ${col}18` }}>
                  <div style={{ fontSize: 16, fontFamily: "var(--font-display)", fontWeight: 800, color: col, lineHeight: 1 }}>{val}</div>
                  <div style={{ fontSize: 8, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", textTransform: "uppercase", marginTop: 3 }}>{label}</div>
                </div>
              ))}
            </div>
            ...
            {(selectedHost.ports?.length ?? 0) > 0 && (
              <div>
                <div style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", textTransform: "uppercase", letterSpacing: "0.9px", marginBottom: 7 }}>
                  Open Ports
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {(selectedHost.ports as any[]).slice(0, 10).map((p: any) => (
                    <div key={`${p.port}-${p.protocol}`} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px",
                      borderRadius: 7, background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", fontWeight: 700, color: "#4d9eff", minWidth: 36 }}>
                        {p.port}
                      </span>
                      <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", textTransform: "uppercase" }}>
                        {p.protocol}
                      </span>
                      <span style={{ flex: 1, fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {p.service}{p.version ? ` — ${p.version}` : ""}
                      </span>
                    </div>
                  ))}
                  {(selectedHost.ports?.length ?? 0) > 10 && (
                    <div style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-quietest)", textAlign: "center" }}>
                      +{selectedHost.ports.length - 10} more — see Open Ports tab
                    </div>
                  )}
                </div>
              </div>
            ...
            {hostVulns.length > 0 && (
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {[...hostVulns].sort((a: any, b: any) => (b.cvss_score ?? 0) - (a.cvss_score ?? 0)).slice(0, 8).map((v: any, vi: number) => {
                    const col = SEV_COLOR[v.severity ?? "info"];
                    return (
                      <div key={vi} style={{ display: "flex", alignItems: "flex-start", gap: 7, padding: "6px 10px",
                        borderRadius: 7, background: "var(--surface-2)", border: `1px solid ${col}18` }}>
                        <span style={{ fontSize: 8, fontFamily: "var(--font-mono)", fontWeight: 800, color: col,
                          background: `${col}18`, border: `1px solid ${col}30`, padding: "1px 5px",
                          borderRadius: 3, flexShrink: 0, textTransform: "uppercase" }}>{v.severity}</span>
                        <span style={{ flex: 1, fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-dim)",
                          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v.title}</span>
                        {v.cvss_score > 0 && (
                          <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", fontWeight: 700, color: col, flexShrink: 0 }}>
            ...
            {hostVulns.length === 0 && (
              <div style={{ padding: "12px 14px", borderRadius: 10, background: "rgba(0,204,136,0.05)",
                border: "1px solid rgba(0,204,136,0.15)", fontSize: 10, fontFamily: "var(--font-mono)", color: "#00cc88" }}>
...
function osAbbr(os?: string): string {
  ...
  if (lc.includes("macos") || lc.includes("darwin") || lc.includes("mac os")) return "MAC";
  if (lc.includes("freebsd") || lc.includes("openbsd") || lc.includes("netbsd")) return "BSD";
  if (lc.includes("cisco") || lc.includes("router") || lc.includes("juniper")) return "NET";
    ...
    <>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
        <defs>
          ...
          <filter id="topoGlowStrong" x="-60%" y="-60%" width="220%" height="220%">
        ...
        {Array.from({ length: 5 }, (_, row) =>
          Array.from({ length: 9 }, (_, col) => (
            <circle key={`dot-${row}-${col}`} cx={col * 40 + 10} cy={row * 46 + 14} r="0.9" fill="rgba(0,229,204,0.07)" />
          ))
        )}
        ...
        {count === 0 && (
          <>
            <circle cx={scanX} cy={H / 2 - 16} r="18" fill="rgba(0,229,204,0.05)"
              stroke="rgba(0,229,204,0.18)" strokeWidth="1" strokeDasharray="3 5" />
            <text x={scanX} y={H / 2 + 10} textAnchor="middle"
              fill="rgba(100,116,130,0.55)" fontSize="10" fontFamily="monospace">
              No hosts discovered
            </text>
          </>
        )}
        ...
        {safeHosts.map((h, i) => {
          ...
          return (
            <g key={`line-${h.ip}`}>
              {/* Base trace */}
              <line x1={scanX} y1={scanY - 20} x2={x} y2={hostY + 22}
                stroke={col} strokeWidth="0.7" opacity="0.18" />
              {/* Animated dashes */}
              <line x1={scanX} y1={scanY - 20} x2={x} y2={hostY + 22}
                stroke={col} strokeWidth="1.1" strokeDasharray="5 10" opacity="0.65"
                style={{ animation: `dash-flow ${1.7 + i * 0.35}s linear infinite` }} />
              {/* Traveling packet */}
              <circle r="2.5" fill={col} filter="url(#topoGlow)">
                <animateMotion dur={`${2.1 + i * 0.4}s`} repeatCount="indefinite"
        ...
        {safeHosts.map((h, i) => {
          ...
          const ip = (h.ip as string) ?? "";
          const rawHost = (h.hostname as string) ?? "";
          const shortHost = rawHost.length > 12 ? rawHost.slice(0, 11) + "…" : rawHost;

          return (
            <g key={`host-${h.ip}`}>
              {/* Pulsing alert ring for critical/high */}
              {isAlert && (
                <circle cx={x} cy={hostY} r="27" fill="none" stroke={col}
                  strokeWidth="1.2" opacity="0.35"
                  style={{ animation: "topo-ring 2.2s ease-out infinite", transformOrigin: `${x}px ${hostY}px` }} />
              )}
              {/* Main host circle */}
              <circle cx={x} cy={hostY} r="20" fill={`${col}18`} stroke={col} strokeWidth="1.6"
                filter={isAlert ? "url(#topoGlowStrong)" : "url(#topoGlow)"} />
              {/* OS abbreviation — top line */}
              <text x={x} y={hostY - 4} textAnchor="middle" fill={col}
                fontSize="8" fontFamily="monospace" fontWeight="800">
                {abbr}
              </text>
              {/* Stats line: vulns · ports */}
              <text x={x} y={hostY + 8} textAnchor="middle" fill={col}
                fontSize="6.5" fontFamily="monospace" opacity="0.8">
                {vulnCnt}v · {portCnt}p
              </text>
              {/* IP address */}
              <text x={x} y={hostY + 36} textAnchor="middle"
                fill="rgba(148,163,184,0.85)" fontSize="7" fontFamily="monospace">
                {ip}
              </text>
              {/* Hostname (if available) */}
              {shortHost && (
                <text x={x} y={hostY + 49} textAnchor="middle"
                  fill="rgba(100,116,130,0.65)" fontSize="6.5" fontFamily="monospace">
                  {shortHost}
                </text>
              )}
            </g>
        ...
        <g>
          <circle cx={scanX} cy={scanY} r="24" fill="none" stroke="rgba(0,229,204,0.15)"
            strokeWidth="1.5" style={{ animation: "topo-pulse 2.6s ease-in-out infinite" }} />
          <circle cx={scanX} cy={scanY} r="17" fill="rgba(0,229,204,0.11)"
            stroke="#00e5cc" strokeWidth="1.6" filter="url(#topoGlow)" />
          <text x={scanX} y={scanY - 2} textAnchor="middle" fill="#00e5cc"
            fontSize="8" fontFamily="monospace" fontWeight="800">
            SXP
          </text>
          <text x={scanX} y={scanY + 9} textAnchor="middle" fill="#00e5cc"
            fontSize="6" fontFamily="monospace" opacity="0.75">
            SCANNER
          </text>
        </g>
      </svg>
        ...
        {LEGEND.map(({ label, col }) => (
          <div key={label} style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: col,
              boxShadow: `0 0 4px ${col}88`, display: "inline-block", flexShrink: 0 }} />
            <span style={{ fontSize: 8, fontFamily: "var(--font-mono)", color: "var(--text-quietest)",
...
export default function NetworkScanPage() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setNetworkMapOpen(false); setSelectedMapHost(null); }
    };
    ...
    >
      ...
      {done && report && (
        ...
        >
          ...
          >
            {[
              { val: report.summary.hosts_discovered, label: "Hosts Found",     col: "#4d9eff", Ic: Globe         },
              { val: report.summary.open_ports,        label: "Open Ports",      col: "#00e5cc", Ic: Layers        },
              { val: report.summary.total_vulns,       label: "Vulnerabilities", col: "#ffcc00", Ic: AlertTriangle },
              { val: report.summary.exploit_count,     label: "Exploits Found",  col: "#ff3355", Ic: Crosshair     },
              ...
              (hosts.length === 0 ? (
                ...
                >
                  ...
                  <div
                    ...
                    onMouseEnter={e => {
                      (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,229,204,.38)";
                      (e.currentTarget as HTMLDivElement).style.boxShadow = "0 0 20px rgba(0,229,204,.08)";
                    }}
                    onMouseLeave={e => {
                      (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,229,204,.16)";
                  ...
                  >
                    <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-dim)",
                      ...
                      <span style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4,
                        fontSize: 9, color: "rgba(0,229,204,0.5)", textTransform: "none", letterSpacing: 0 }}>
                        <Maximize2 size={9} color="rgba(0,229,204,0.5)" /> expand
                      </span>
                    </div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    <button
                      onClick={() => {
                        const prefix = `SCN-${report.scan_id.slice(-6).toUpperCase()}`;
                        downloadHTML(buildReportFromScanReport(report, undefined, vulns), `${prefix}-report.html`);
                      }}
                      style={{
                        padding: "10px 20px",
                        borderRadius: 9,
                    ...
                    <button
                      onClick={() => {
                        openPrintPDF(buildReportFromScanReport(report, undefined, vulns));
                      }}
                      style={{
                        padding: "10px 20px",
                        borderRadius: 9,
      ...
      {networkMapOpen && (
        <div
          onClick={() => { setNetworkMapOpen(false); setSelectedMapHost(null); }}
          style={{
            position: "fixed", inset: 0, zIndex: 1200,
            background: "rgba(0,0,0,0.72)", backdropFilter: "blur(6px)",
            display: "flex", alignItems: "center", justifyContent: "center",
            padding: 24, animation: "fade-in .18s ease",
          }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{
              width: "min(92vw, 1080px)", height: "min(85vh, 680px)",
              background: "var(--surface-0)",
              border: "1px solid rgba(0,229,204,0.22)",
              borderRadius: 18, overflow: "hidden",
              display: "flex", flexDirection: "column",
              boxShadow: "0 0 0 1px rgba(0,229,204,0.08), 0 32px 80px rgba(0,0,0,0.6)",
          ...
          >
            ...
            <div style={{
              ...
              background: "rgba(0,229,204,0.03)", flexShrink: 0,
            }}>
              <Network size={14} color="#00e5cc" style={{ filter: "drop-shadow(0 0 5px #00e5ccaa)" }} />
              <span style={{ fontSize: 13, fontFamily: "var(--font-display)", fontWeight: 700, color: "var(--text-body)" }}>
                Network Topology Map
              </span>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8 }}>
                <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-quietest)",
                  background: "rgba(0,229,204,0.08)", border: "1px solid rgba(0,229,204,0.15)",
                  padding: "2px 8px", borderRadius: 5 }}>
                  {hosts.length} host{hosts.length !== 1 ? "s" : ""}
                </span>
                <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-quietest)",
                  background: "rgba(255,204,0,0.08)", border: "1px solid rgba(255,204,0,0.15)",
                  padding: "2px 8px", borderRadius: 5 }}>
                  {vulns.length} vuln{vulns.length !== 1 ? "s" : ""}
                </span>
              </div>
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "rgba(100,116,130,0.6)" }}>
                  ESC or click outside to close
                </span>
                <button
                  onClick={() => { setNetworkMapOpen(false); setSelectedMapHost(null); }}
                  style={{ width: 28, height: 28, borderRadius: 8, border: "1px solid rgba(255,255,255,0.08)",
                    background: "rgba(255,255,255,0.04)", display: "flex", alignItems: "center",
                    justifyContent: "center", cursor: "pointer", transition: "background .15s ease",
                    color: "var(--text-dim)" }}
