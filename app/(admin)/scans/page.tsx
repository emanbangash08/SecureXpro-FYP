...
import { Search, ChevronRight, Filter, Activity, Server, Globe, CheckCircle2, AlertCircle, Clock, Trash2, ExternalLink } from 'lucide-react'
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#8899aa', fontFamily: 'var(--font-mono)' }}>
            Monitor and manage all security assessments across network and web applications.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'flex', padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,.05)', background: '#050709' }}>
            ...
            <div style={{ width: 150, fontSize: 11, fontFamily: 'var(--font-mono)', color: '#4a5568', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'right' }}>Actions</div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {filteredScans.map(scan => (
                <div key={scan.id} style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,.04)', transition: 'background .2s', cursor: 'default' }}
                  ...
                  <div style={{ width: 150, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                    <Link href={`/scans/${scan.type}/${scan.id}`}>
                      <button style={{ padding: '6px 10px', borderRadius: 6, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.1)', color: '#e8edf5', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontFamily: 'var(--font-mono)', transition: 'all .2s' }}>
                        <ExternalLink size={14} /> View
                      </button>
                    </Link>
                    <button style={{ padding: '6px', borderRadius: 6, background: 'transparent', border: '1px solid transparent', color: '#4a5568', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all .2s' }}
