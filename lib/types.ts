...
export interface ApiScan {
  ...
  created_at: string;
  // Distributed scanning — populated when the scan is dispatched to a remote agent.
...
export interface ScanCreatePayload {
  ...
  scan_type: ApiScanType;
  options: Partial<ApiScanOptions>;
  /** Optional — when set, the scan waits in PENDING_AGENT until the remote
   *  agent (a user with role=agent) polls and runs Nmap inside its network. */
  assigned_agent_id?: string | null;
}
